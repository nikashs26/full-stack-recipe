#!/usr/bin/env python3
"""
Generate an optimized Railway app that loads recipes from a separate JSON file
This will be more memory efficient and faster to deploy
"""
...
def generate_optimized_railway_app():
    """Generate optimized Railway app with separate recipe data file"""
    print("🚀 Generating optimized Railway app with separate recipe data...")
    ...
    try:
        ...
        for i, (recipe_id, metadata, document) in enumerate(zip(all_recipes['ids'], all_recipes['metadatas'], all_recipes['documents'])):
            try:
                ...
                # Ensure required fields exist with minimal data
                recipe = {
                    'id': recipe_id,
        ...
        # Save recipes to separate JSON file
        recipes_file = 'backend/recipes_data.json'
        with open(recipes_file, 'w') as f:
            json.dump(recipes, f, indent=2)
        
        print(f"   💾 Saved recipes to {recipes_file}")
        
        # Generate the optimized Railway app file
...
'''
        ...
        # Write the optimized Railway app file
        output_file = 'backend/railway_app_optimized.py'
        with open(output_file, 'w') as f:
            f.write(app_content)
        
        print(f"✅ Generated optimized Railway app file: {output_file}")
        print(f"📊 App file size: {os.path.getsize(output_file)} bytes")
        print(f"📊 Recipes data file size: {os.path.getsize(recipes_file)} bytes")
        
        return True
        
    except Exception as e:
        print(f"❌ Error generating optimized Railway app: {e}")
        return False

if __name__ == "__main__":
    generate_optimized_railway_app()
