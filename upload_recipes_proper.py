#!/usr/bin/env python3
"""
Upload all local recipes to Railway with proper structure and metadata
This script preserves all the original tags, metadata, and structure
"""
...
class ProperRecipeUploader:
    def __init__(self, railway_url: str = "https://full-stack-recipe-production.up.railway.app"):
        ...
        print(f"🚀 Proper Recipe Uploader initialized")
        print(f"   Railway URL: {self.railway_url}")
        print(f"   API URL: {self.api_url}")
    
    def get_all_local_recipes(self):
        """Get all recipes from local ChromaDB with full metadata"""
        try:
            print("📊 Fetching all recipes from local ChromaDB with full metadata...")
            ...
            # Process recipes with full metadata
            processed_recipes = []
            ...
            print(f"   ✅ Successfully processed {len(processed_recipes)} recipes with full metadata")
    ...
    def clear_railway_recipes(self):
        """Clear existing recipes on Railway"""
        try:
            print("🗑️  Clearing existing recipes on Railway...")
            response = requests.post(f"{self.api_url}/clear-recipes", timeout=30)
            if response.status_code == 200:
                print("   ✅ Railway recipes cleared")
                return True
            else:
                print(f"   ⚠️  Could not clear recipes: {response.status_code}")
                return False
        except Exception as e:
            print(f"   ⚠️  Error clearing recipes: {e}")
            return False
    ...
    def run_upload(self):
        ...
        print("🚀 Starting Proper Recipe Upload to Railway")
        print("=" * 60)
        
        # Step 1: Get all local recipes with full metadata
        print("\n1️⃣ Fetching local recipes with full metadata...")
        ...
        # Step 3: Clear existing recipes
        print("\n3️⃣ Clearing existing recipes...")
        self.clear_railway_recipes()
        
        # Step 4: Upload recipes
        print("\n4️⃣ Uploading recipes with full metadata...")
        ...
        try:
            ...
            if verify_response.status_code == 200:
                ...
                search_response = requests.post(
                    ...
                    json={"search": "chicken", "limit": 3},
                    headers={'Content-Type': 'application/json'},
                    timeout=10
                )
                if search_response.status_code == 200:
                    search_data = search_response.json()
                    print(f"   🔍 Search test: Found {search_data.get('total', 0)} chicken recipes")
                else:
                    print(f"   ⚠️  Search test failed: {search_response.status_code}")
            else:
        ...
        if uploaded_count > 0:
            print(f"\n🎉 SUCCESS! {uploaded_count} recipes uploaded with full metadata!")
